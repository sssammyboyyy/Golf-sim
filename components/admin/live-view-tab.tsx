...
export function LiveViewTab() {
  ...
  const handleOpenCreate = (prefillBayId: number = 1) => {
    ...
    setSelectedBooking({
      ...
      status: 'confirmed', payment_type: 'pending', payment_status: 'pending', user_type: 'walk_in',
    ...
    <div className="w-full space-y-6 max-w-6xl mx-auto px-4 pb-20 mt-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end pb-8 border-b-2 border-zinc-800 gap-6">
        ...
        <div className="flex flex-col items-stretch gap-4 w-full md:w-auto">
          <Button onClick={() => handleOpenCreate(1)} className="bg-white text-black hover:bg-primary hover:text-white font-black uppercase text-xs h-14 min-h-[56px] shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-all w-full md:w-auto">
          ...
          <div className="flex items-center justify-between md:justify-start bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-2xl h-14 md:h-auto">
            <button onClick={() => shiftDate(-1)} className="p-4 hover:bg-zinc-800 transition-all text-zinc-500 hover:text-white border-r border-zinc-800"><ChevronLeft size={20} /></button>
            <div className="flex-1 md:flex-none flex items-center justify-center gap-3 px-6 py-2 bg-black/40">
              <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent text-sm font-black text-white outline-none cursor-pointer uppercase" />
            </div>
            <button onClick={() => shiftDate(1)} className="p-4 hover:bg-zinc-800 transition-all text-zinc-500 hover:text-white border-l border-zinc-800"><ChevronRight size={20} /></button>
            <button onClick={() => setSelectedDate(getSASTDate())} className="px-5 py-3 text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary hover:bg-primary hover:text-white border-l border-zinc-800 h-full hidden md:flex items-center">TODAY</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          ...
          <div key={m.label} className={`relative overflow-hidden bg-[#0a0a0a] ${m.border} border border-zinc-800 rounded-2xl p-4`}>
            <div className="absolute top-[-8px] right-[-8px] opacity-[0.07] pointer-events-none"><m.icon size={60} /></div>
            <div className="flex items-center gap-2 mb-1"><m.icon size={12} className={m.color} /><span className="text-[9px] font-black uppercase tracking-[0.25em] text-zinc-500">{m.label}</span></div>
            <span className={`text-xl md:text-3xl font-black tabular-nums tracking-tighter ${m.color}`}>{m.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col space-y-4 min-h-[450px]">
        {isLoading ? (
          <div className="py-32 text-center animate-pulse text-zinc-600 font-black uppercase tracking-[0.8em] text-xs">Syncing Ledger...</div>
        ) : data.length === 0 ? (
          <div className="py-32 border border-dashed border-zinc-800 bg-zinc-900/10 rounded-3xl text-center px-4">
          ...
          data.map((booking) => {
            ...
            return (
              <div key={booking.id} onClick={() => { setSelectedBooking(booking); setIsModalOpen(true); }} className={`group relative flex flex-col md:flex-row items-stretch md:items-center justify-between p-4 md:p-6 bg-[#0a0a0a] ${bay.border} border border-zinc-800/80 rounded-2xl hover:bg-zinc-800/40 hover:border-primary/50 transition-all cursor-pointer shadow-lg`}>
                <div className={`absolute left-0 top-0 bottom-0 w-2 ${booking.amount_due > 0 && Number(booking.amount_paid) > 0 ? 'bg-amber-500' : booking.amount_due > 0 ? 'bg-red-500' : 'bg-emerald-500'} ${booking.amount_due > 0 && !paid ? 'animate-pulse' : ''}`} />
                ...
                <div className="flex flex-row items-center gap-4 md:gap-8 w-full md:w-auto pl-4 mb-4 md:mb-0">
                  <div className="flex flex-col items-center justify-center min-w-[70px] md:min-w-[90px] border-r border-zinc-800/50 pr-4 md:pr-8">
                    ...
                    <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest mt-2">Start</span>
                  </div>
                  <div className="flex flex-col">
                    <span className={`text-lg md:text-xl font-black uppercase italic tracking-tighter ${bay.text}`}>{bay.name}</span>
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-tight">{booking.player_count}P // {booking.duration_hours}H</span>
                  </div>
                </div>
                ...
                <div className="flex flex-row items-center gap-4 md:gap-6 w-full md:w-auto justify-between md:justify-end px-2 md:px-0 flex-shrink-0">
                  <div className="flex flex-col items-end bg-[#131313] p-3 rounded-xl border border-zinc-800/80 min-w-[150px] shadow-inner">
                    <div className="flex justify-between items-end w-full mb-2 border-b border-white/5 pb-2">
                      <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">TOTAL</span>
                      <span className="text-3xl font-black text-white tabular-nums tracking-tighter leading-none">R{booking.total_price ?? 0}</span>
                    </div>
                    <div className="flex justify-between w-full text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">
                      <span>Paid:</span> <span className="text-emerald-400">R{booking.amount_paid || 0}</span>
                    </div>
                    <div className="flex justify-between items-center w-full">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Due:</span>
                      <span className={`text-[11px] font-black uppercase tracking-widest ${booking.amount_due > 0 ? 'text-amber-500' : 'text-zinc-500'}`}>R{booking.amount_due ?? 0}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 md:gap-3">
                    {paid ? (
                      <Button size="sm" className="bg-emerald-500/20 hover:bg-emerald-500 text-emerald-400 hover:text-white border border-emerald-500/30 font-black text-[9px] uppercase h-14 px-5 transition-all rounded-xl" onClick={(e) => { e.stopPropagation(); handleQuickSettle(booking, 'unsettle'); }}>
                        <CheckCircle className="w-4 h-4 mr-1.5" /> Paid
                      </Button>
                    ) : (
                      <Button size="sm" className="bg-red-600 hover:bg-red-500 text-white font-black text-[10px] uppercase h-14 px-6 shadow-xl transition-all rounded-xl animate-pulse" onClick={(e) => { e.stopPropagation(); handleQuickSettle(booking, 'settle'); }}>
                        <Plus className="w-4 h-4 mr-1.5" /> Settle
                      </Button>
                    )}
                    <div className="flex flex-col items-center gap-1.5 bg-zinc-900/50 p-1 rounded-2xl border border-zinc-800">
                       <button onClick={(e) => { e.stopPropagation(); handleQuickExtend(booking, 0.5); }} className="text-[9px] font-black text-primary hover:text-white bg-primary/10 hover:bg-primary px-3 py-1.5 rounded-xl transition-all">+30m</button>
                       <button onClick={(e) => { e.stopPropagation(); handleQuickExtend(booking, 1); }} className="text-[9px] font-black text-primary hover:text-white bg-primary/10 hover:bg-primary px-3 py-1.5 rounded-xl transition-all">+1H</button>
                    </div>
                  </div>
                </div>
              </div>
